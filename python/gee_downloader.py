...
Objective: Download and post-process Google Earth Engine composites.
Author: Yi Yu (yi.yu1@sydney.edu.au)
Created: 2026-02-17
Last updated: 2026-03-17
Inputs: YAML configuration file, Earth Engine authentication, extent plus image/image-collection settings.
Outputs: Downloaded GeoTIFF composites with standardized band metadata and post-processing updates.
Usage: python gee_downloader.py <config.yaml>
Dependencies: earthengine-api, requests, pyyaml, python-dateutil
Optional dependencies for GeoTIFF post-processing: rasterio, numpy
"""
import os
import sys
import yaml
...
def _postprocess_geotiff(
    ...
    if rasterio is None or np is None:
        print("[post] rasterio/numpy not available; skipping optional GeoTIFF post-processing.")
        return
...
class GEEDownloader:
    ...
    def _validate_config(self):
        self.cfg.setdefault("project_id", os.environ.get("GEE_PROJECT_ID"))
        has_collection = "collections" in self.cfg or "collection" in self.cfg
        has_image = "image" in self.cfg
        if has_collection == has_image:
            raise ValueError("Provide exactly one data source: collection/collections or image.")

        req = [
            "coords", "download_dir", "bands", "scale", "out_format",
            "auth_mode", "filename_prefix", "project_id"
        ]
        if has_collection:
            req.extend([
        ...
        self.cfg["download_dir"] = os.path.abspath(os.path.expanduser(self.cfg["download_dir"]))
        self.cfg["project_id"] = str(self.cfg["project_id"]).strip()
        if not self.cfg["project_id"]:
            raise ValueError("project_id is required. Set it in the YAML config or via GEE_PROJECT_ID.")
        self.cfg["bands"] = self._normalize_bands(self.cfg["bands"])
        if has_collection:
            self.cfg["source_type"] = "collection"
            ...
            self.cfg["source_type"] = "image"
            self.cfg["image"] = str(self.cfg["image"]).strip()
            if not self.cfg["image"]:
                raise ValueError("image must be a non-empty Earth Engine image asset ID.")
    ...
    @staticmethod
    def _normalize_bands(raw):
        if isinstance(raw, str):
            raw = [raw]
        if not isinstance(raw, list) or len(raw) == 0:
            raise ValueError("bands must be a non-empty string or list of strings.")

        out = []
        for item in raw:
            if not isinstance(item, str) or not item.strip():
                raise ValueError("bands entries must be non-empty strings.")
    ...
    def _image_asset(self) -> ee.Image:
        img = ee.Image(self.cfg["image"])
        if (self.cfg.get("cloud_mask") or {}).get("enabled", False):
            mask = build_mask_condition(img, self.cfg)
    ...
    def run(self):
        ...
        if self.cfg["source_type"] == "image":
            self._run_single_image_download()
            return
    ...
    def _run_single_image_download(self):
        image_id = self.cfg["image"]
        out_name = f"{self.cfg['filename_prefix']}.{self.cfg['out_format']}"
        out_final = os.path.join(self.cfg["download_dir"], out_name)
        image_tag = image_id.replace("/", "_")
        out_tmp = os.path.join(self.cfg["download_dir"], f"__tmp_{image_tag}_{out_name}")

        if os.path.exists(out_final):
            print(f"skip existing file {out_name}")
            return

        print(f"Using image asset: {image_id}")

        img = self._image_asset()
        ...
        os.replace(out_tmp, out_final)
        print(f"Done. Success=1, Failed=0, SkippedDuplicates=0")
...
def main():
    if len(sys.argv) < 2:
        print("Usage: python gee_downloader.py <config.yaml>")
        sys.exit(1)
    try:
